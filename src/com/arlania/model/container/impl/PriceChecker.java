...
public class PriceChecker extends ItemContainer {
	...
	public PriceChecker open() {
		...
		getPlayer().getPacketSender().sendMessage("Note: When selling an item to a store, it loses 25% of its original value!").sendMessage("Prices shown in the price-checker are the original values!");
		refreshItems();
		open = true;
		return this;
	}
		...
		for(int i = 0; i < frames.length; i++) {
			...
			if(slotOccupied) {
				...
				if(it.getDefinition().isStackable()) {
					...
					long value = it.getDefinition().getValue();
					long amount = it.getAmount();
					long totalPrice = value * amount;
					int frame = getFrame(i, 1);
					getPlayer().getPacketSender().sendString(frame, "" + it.getDefinition().getValue() + " x" + itemAmount);
					getPlayer().getPacketSender().sendString(frame + 1, "= " + Misc.insertCommasToNumber(Long.toString(totalPrice)));
					getPlayer().getPacketSender().sendItemOnInterface(getFrame(i, 2), it.getId(), itemAmount);
				} else {
					getPlayer().getPacketSender().sendString(getFrame(i, 1), "" + Misc.insertCommasToNumber(Long.toString(it.getDefinition().getValue())) + "");
			...
			getPlayer().getPacketSender().sendString(18351, "" + Misc.insertCommasToNumber(Long.toString(calculateTotalWealth())));
		}
		if(itemFound) {
	...
	public PriceChecker switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
		...
		if(to instanceof BeastOfBurden || to instanceof PriceChecker) {
			if(item.getAmount() > to.freeSlots() && !(item.getDefinition().isStackable() && to.contains(item.getId()))) {
				item.setAmount(to.freeSlots());
			}
		}
		if(item.getAmount() <= 0)
			return this;
	...
	public long calculateTotalWealth() {
		...
		for(Item item : getValidItems()) {
			long value = item.getDefinition().getValue();
	...
	private static final int frames[][] = {{0, 18353, 18246}, {1, 18356, 18500}, {2, 18359, 18501}, {3, 18362, 18502}, {4, 18365, 18503}, {5, 18368, 18504}, {6, 18371, 18505}, {7, 18374, 18506}, {8, 18377, 18507}, {9, 18380, 18508}, {10, 18383, 18509}, {11, 18386, 18510}, {12, 18389, 18511}, {13, 18392, 18512}, {14, 18395, 18513}, {15, 18398, 18514}, {16, 18401, 18515}, {17, 18404, 18516}, {18, 18407, 18517}, {19, 18410, 18518}};
