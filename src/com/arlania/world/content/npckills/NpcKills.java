...
public class NpcKills {

	/** this is your starting variable for tracking **/
	private static int npcsKilled = 0;
	...
	private static void setNpcsKilled(int killed) {
		npcsKilled = killed;
	}
	
	public static int getTotalNpcsKilled() {
		return npcsKilled;
	}
	...
	public static void save() {
		...
		Path path = Paths.get("./data/", "npckills.json");
		...
		try (FileWriter writer = new FileWriter(file)) {
			...
			object.addProperty("killednpcs", npcsKilled + 1);
	...
	public static void getResult() {
		...
		try (FileReader fileReader = new FileReader(file)) {
			...
			if (reader.has("killednpcs")) {
                setNpcsKilled(reader.get("killednpcs").getAsInt());
            }
